...
`render.yaml` يعرّف خدمتين على الخطة المجانية بدل حاوية Docker واحدة:
...
**لماذا التقسيم لا Docker:** حدّ الـ512MB يُطبَّق على كل خدمة على حدة، فالتقسيم يضاعف الميزانية مجانًا. الحاوية الواحدة كانت تُقتل بتجاوز الحدّ، ولم تكن تشغّل `qcluster` أصلًا — أي أن **كل المهام الخلفية (بريد التحقق، الحملات، النشر المجدول) لم تكن تُنفَّذ في الإنتاج**. الآن العامل يعمل داخل خدمة الـ API، وسقوطه يُسقط الخدمة فيعيد Render تشغيلها.

**كيف تتصل الخدمتان:** الخدمات المجانية لا تستقبل على الشبكة الخاصة، فالواجهة تصل إلى الـ API عبر عنوانه العام (`BACKEND_ORIGIN=https://nexa-api.onrender.com`). المتصفح لا يعرف إلا نطاق الواجهة: إعادة الكتابة في `next.config` تمرّر `/api` و`/admin` و`/media` كما في السابق.
...
**بعد أول نشر:**
1. اضبط `REVALIDATE_SECRET` بالقيمة نفسها في الخدمتين (معلَّم `sync: false` فلا يولَّد آليًا).
2. أضف النطاق المخصص إلى `nexa-web` فقط؛ `nexa-api` يبقى على `onrender.com`.
3. `EMAIL_BACKEND` ما زال `console` حتى يُحسم مزود البريد.

**ما يبقى مفتوحًا على الخطة المجانية:** لا قرص دائم، فالوسائط المرفوعة تضيع مع كل نشر — الحل قرص Render مدفوع أو تخزين S3-متوافق. والخدمتان تنامان بعد الخمول فأول طلب يتأخر.
