...
نشر تقليدي على VPS واحد (أو خدمتان على Render — انظر أدناه) — بلا Docker. المكوّنات: Nginx (عكسي + SSL) →
...
## النشر على Render (خدمتان بلا Docker)

`render.yaml` يعرّف خدمتين (خطة `0.5c-512mb`، ~$7 لكل خدمة) بدل حاوية Docker واحدة:

| الخدمة | Runtime | ما تشغّله | الذاكرة المقيسة محليًا |
|---|---|---|---|
| `nexa-api` | Python | Gunicorn (عامل × خيطان) + `qcluster` (عامل واحد) — `backend/start.sh` | ~165MB من 512MB |
| `nexa-web` | Node | `next start` بكومة V8 مسقوفة عند 384MB — `frontend/scripts/start.sh` | ~300MB من 512MB |

**لماذا التقسيم لا Docker:** حدّ الـ512MB يُطبَّق على كل خدمة على حدة، فالتقسيم يضاعف الميزانية بخدمة ثانية بالسعر نفسه (بدل القفز إلى خطة 2GB بـ $25). الحاوية الواحدة كانت تُقتل بتجاوز الحدّ، ولم تكن تشغّل `qcluster` أصلًا — أي أن **كل المهام الخلفية (بريد التحقق، الحملات، النشر المجدول) لم تكن تُنفَّذ في الإنتاج**. الآن العامل يعمل داخل خدمة الـ API، وسقوطه يُسقط الخدمة فيعيد Render تشغيلها.

**كيف تتصل الخدمتان:** عبر شبكة Render الخاصة (`BACKEND_ORIGIN=http://nexa-api:10000`) — لا يخرج الطلب إلى الإنترنت. المتصفح لا يعرف إلا نطاق الواجهة: إعادة الكتابة في `next.config` تمرّر `/api` و`/admin` و`/media` كما في السابق.

**البناء بلا API:** خدمة الواجهة تُبنى بـ `NEXT_BUILD_OFFLINE=true` فلا تنتظر Django؛ الفهارس تُولَّد فارغة وتُبطَل فور الإقلاع (كما كان يحدث مع البناء على قاعدة فارغة)، وصفحات التفاصيل تُولَّد عند الطلب.

**قاعدة البيانات القائمة لا تُدار من الملف** عمدًا (ذكرها يعني تطبيق خطة الملف عليها). عند إنشاء Blueprint:
1. انسخ `DB_NAME` و`DB_USER` و`DB_PASSWORD` و`DB_HOST` من إعدادات الخدمة القديمة `nexa` إلى `nexa-api` — القاعدة نفسها بلا ترحيل.
2. اضبط `REVALIDATE_SECRET` بالقيمة نفسها في الخدمتين (معلَّم `sync: false` فلا يولَّد آليًا).
3. انقل النطاق المخصص إلى `nexa-web`؛ `nexa-api` لا يحتاج نطاقًا.
4. تحقق في سجل `nexa-api` من `Q Cluster ... running`، ثم احذف خدمة `nexa` القديمة.
5. `EMAIL_BACKEND` ما زال `console` حتى يُحسم مزود البريد.

**ما يبقى مفتوحًا:** لا قرص دائم على الخدمات، فالوسائط المرفوعة تضيع مع كل نشر — الحل قرص Render (يُربط بـ `nexa-api` على `/opt/render/project/src/media`) أو تخزين S3-متوافق.
...
- [ ] النشرة: الاشتراك من الرئيسية يصل بريد التأكيد إلى Gmail وOutlook في الوارد لا السبام، وحملة اختبارية (`test-send`) كذلك — الإرسال الدفعي لا يبدأ قبل ذلك.
