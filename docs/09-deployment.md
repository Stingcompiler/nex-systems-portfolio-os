...
## النشر على Render (خدمة واحدة + Cron Job)

الخدمة القائمة `nex-systems-portfolio-os-1` من نوع Docker على خطة 512MB (~$7). كانت تُقتل بنفاد الذاكرة ولا تشغّل عامل المهام إطلاقًا. الإعداد الحالي يبقيها خدمة واحدة بثلاثة تنازلات مقاسة:

| الإجراء | الأثر المقيس |
|---|---|
| كومة Node 128MB بدل 224 (`NODE_HEAP_MB`) | 185MB RSS بدل 250 بعد 45 صفحة، بلا أخطاء |
| `images.unoptimized` في Next | يزيل معالجة `sharp` داخل Node (ذروات 100MB+) — Django يولّد WebP أصلًا |
| عامل المهام كـ Cron Job كل 5 دقائق (`process_queue`) | يزيل عملية `qcluster` الدائمة (~70MB) |

المجموع ~310MB بدل ~445MB، أي هامش ~200MB. البديل الأوسع هامشًا — خدمتان Python وNode ($14) — موثَّق في تاريخ `render.yaml` (PR #10/#13) ويُعاد إليه بلا تعديل كود إن ضاق هذا.

**إعداد الـ Cron Job في اللوحة (New → Cron Job):**
- المستودع نفسه، Runtime **Python 3**، Root Directory `backend`.
- Build: `pip install -r requirements/prod.txt` · Command: `python manage.py process_queue` · Schedule: `*/5 * * * *`.
- متغيرات البيئة كما في الخدمة (`DJANGO_SETTINGS_MODULE`, `SECRET_KEY`, `DB_*`, `FRONTEND_URL`, `REVALIDATE_SECRET`, `EMAIL_*`) — الأسهل Environment Group مشترك. أضف `Q_WORKERS=1`.
- الكلفة: حد أدنى $1/شهر، وتُحاسَب بالثانية (الدورة ~6 ثوانٍ عندما يكون الطابور صغيرًا).

**ما يعنيه هذا عمليًا:** بريد التحقق وتأكيدات الطلبات وإرسال الحملات تنطلق في الدورة التالية — تأخير حتى 5 دقائق. `process_queue` يشغّل المُجدوِل أولًا (المهام الدورية المستحقة) ثم يُصفّي الطابور ويخرج عندما يفرغ، بسقف 240 ثانية كي لا تتراكب الدورات.

**في الخدمة الرئيسية:** أضف `NODE_HEAP_MB=128` (اختياري — هو الافتراضي في `start.sh`).
...
- [ ] عامل المهام يعمل: على VPS عبر `qcluster`، وعلى Render عبر Cron Job `process_queue` (تحقق من Success في `/admin/django_q/`).
